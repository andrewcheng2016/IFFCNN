...
def find_kernels_ipca(dataset, sample_patches, dc, num_kernels, Q, sqs, m, n, use_gpu, print_detail):
    ...
    principal_components, latent_new, Q, m, sqs, sigma_new, z_new, n, gamma  = IPCA(Q, n, training_data, m, sqs, use_gpu=use_gpu)
...
def multi_Saab_transform_IPCA(dataset, images, labels, kernel_sizes, num_kernels, pca_params,
                              use_num_images, use_classes, random_seed, print_detail, layer_no, use_gpu):
    '''
	Do the PCA "training".
	:param images: [num_images, height, width, channel]
	:param labels: [num_images]
    ...
    kernels, mean, dc_mean, Q, mean, sqs, n, std, gamma, z, pc = find_kernels_ipca(dataset, sample_patches,
...
def initialize(sample_images, layer_no, pca_params, print_detail=False):
    ...
    # mean = pca_params['Layer_%d/mean_old' % i]
    # std = pca_params['Layer_%d/std' % i]
    current_kernel_sizes = kernel_sizes if isinstance(kernel_sizes, int) else kernel_sizes[i]
    ...
    # Flatten
    sample_patches = sample_patches.reshape([-1, sample_patches.shape[-1]])
    # sample_patches = np.subtract(sample_patches, mean) / std
